...
class MainApp(QMainWindow):
    ...
    # ---------- UI helpers ----------
    def _slider_with_lineedit(self, min_val, max_val, default_val) -> QWidget:
        """
        Returns a QWidget containing a QSlider + QLineEdit.
        Also attaches the *slider* as `widget._slider` so we can access it easily.
        """
        w = QWidget()
        lay = QHBoxLayout(w)
        ...
        e = QLineEdit(str(default_val))
        e.setValidator(QIntValidator(min_val, max_val, self))
        e.setFixedWidth(55)

        # sync
        s.valueChanged.connect(lambda v: e.setText(str(v)))
        e.editingFinished.connect(lambda: s.setValue(int(e.text())))

        lay.addWidget(s, 1)
        lay.addWidget(e, 0)

        # expose slider for convenience
        w._slider = s
        return w

    def _create_clone_sliders(self):
        """Concrete tuple (no generators)."""
        resize = self._slider_with_lineedit(10, 300, 100)     # %
        rotate = self._slider_with_lineedit(-180, 180, 0)     # deg
        strength = self._slider_with_lineedit(0, 100, 50)     # %
        feather = self._slider_with_lineedit(0, 100, 20)      # px-ish
        return resize, rotate, strength, feather

    def _create_lighting_sliders(self):
        """Concrete tuple (no generators)."""
        angle = self._slider_with_lineedit(0, 360, 45)        # deg
        intensity = self._slider_with_lineedit(0, 100, 75)    # %
        smooth = self._slider_with_lineedit(10, 300, 100)     # exponent-ish
        return angle, intensity, smooth
    ...
    def initUI(self):
        ...
        # ---------------- Cloning Tab ----------------
        cloning_tab = QWidget()
        cloning_v = QVBoxLayout(cloning_tab)

        # checkbox row
        row0 = QHBoxLayout()
        self.lighting_checkbox = QCheckBox("Enable Source Lighting")
        row0.addWidget(self.lighting_checkbox)
        row0.addStretch(1)
        cloning_v.addLayout(row0)

        # sliders (create once, reuse)
        (
            self.resize_slider_w,
            self.rotate_slider_w,
            self.strength_slider_w,
            self.feather_slider_w,
        ) = self._create_clone_sliders()
        ...
        def add_row(label_text, widget):
            ...
            row.addWidget(lbl)
            row.addWidget(widget, 1)  # make slider stretch
            cloning_v.addLayout(row)

        add_row("Scale:", self.resize_slider_w)
        add_row("Rotate:", self.rotate_slider_w)
        add_row("Intensity:", self.strength_slider_w)
        add_row("Feather:", self.feather_slider_w)

        self.tabs.addTab(cloning_tab, "Cloning Controls")

        # ---------------- Lighting Tab ----------------
        lighting_tab = QWidget()
        lighting_v = QVBoxLayout(lighting_tab)

        self.btn_color = QPushButton("Choose Light Color")
        rowc = QHBoxLayout()
        rowc.addWidget(self.btn_color)
        rowc.addStretch(1)
        lighting_v.addLayout(rowc)

        rowb = QHBoxLayout()
        rowb.addWidget(QLabel("Light Blend:"))
        self.light_blend_combo = QComboBox()
        self.light_blend_combo.addItems(
            ["Overlay", "Soft Light", "Screen (Lighten)", "Multiply (Darken)"]
        )
        rowb.addWidget(self.light_blend_combo)
        rowb.addStretch(1)
        lighting_v.addLayout(rowb)

        self.angle_slider_w, self.intensity_slider_w, self.smoothness_slider_w = (
            self._create_lighting_sliders()
        )

        add_row_l = lambda text, w: (
            (lambda r=QHBoxLayout(): (r.addWidget(QLabel(text)), r.addWidget(w, 1), lighting_v.addLayout(r))[-1])  # noqa
        )
        add_row_l("Light Angle:", self.angle_slider_w)
        add_row_l("Light Intensity:", self.intensity_slider_w)
        add_row_l("Gradient Smoothness:", self.smoothness_slider_w)

        self.tabs.addTab(lighting_tab, "Source Lighting")
        self.tabs.setTabEnabled(1, False)  # disabled until checkbox checked

        # --- Connections ---
        self.lighting_checkbox.toggled.connect(self.toggle_lighting_tab)

        # pass the actual QSliders to the preview label
        self.dest_label.set_preview_dependencies(
            self.src_label, self.resize_slider_w._slider, self.rotate_slider_w._slider
        )
        ...
        for w in [self.angle_slider_w, self.intensity_slider_w, self.smoothness_slider_w]:
            w._slider.valueChanged.connect(self._apply_lighting_to_source)
    ...
    def load_source(self):
        ...
        if file_path:
            ...
            if self.src_img_original is None:
                ...
                return
            if (
                len(self.src_img_original.shape) < 3
                or self.src_img_original.shape[2] != 4
            ):
                QMessageBox.warning(
                    self,
                    "Warning",
    ...
    def load_destination(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Open Destination Image", "", "Image Files (*.png *.jpg *.jpeg)"
        )
        if file_path:
    ...
    def _apply_lighting_to_source(self):
        ...
        if (
            len(self.src_img_original.shape) == 3
            and self.src_img_original.shape[2] == 4
        ):
            has_alpha = True
        ...
        angle = self.angle_slider_w._slider.value()
        intensity = self.intensity_slider_w._slider.value()
        smoothness = self.smoothness_slider_w._slider.value()
        blend_mode = self.light_blend_combo.currentText()
        h, w = base_image_bgr.shape[:2]

        gradient_bgra = self._create_gradient(
            w, h, angle, self.light_color, intensity, smoothness
        )
        ...
        alpha_norm = cv2.cvtColor(
            gradient_alpha.astype(np.float32) / 255.0, cv2.COLOR_GRAY2BGR
        )
        processed_bgr = (
    ...
    def _create_gradient(self, w, h, angle, color_bgr, intensity, smoothness):
        ...
        alpha_channel = (
            gradient_norm ** exponent * (intensity / 100.0) * 255
        ).astype(np.uint8)
    ...
    def run_cloning(self):
        ...
        strength = self.strength_slider_w._slider.value() / 100.0
        feather_amount = self.feather_slider_w._slider.value()

        if self.src_img_lit is None or self.dest_img is None or center_point is None:
            QMessageBox.warning(
                self, "Error", "Please load images and click a destination point."
            )
            return
        ...
        try:
            ...
            if clone_mode_str.startswith("Alpha Blend"):
                ...
                self.result_img = cv2.addWeighted(
                    cloned_at_100, strength, self.dest_img, 1 - strength, 0
                )
    ...
    def _rotate_image_and_mask(self, image_bgra, mask, angle_deg):
        ...
        rotated_mask = cv2.warpAffine(
            mask, M, (new_w, new_h), flags=cv2.INTER_NEAREST, borderValue=0
        )
    ...
    def display_image(self, img_cv, label):
        ...
        if len(img.shape) == 3 and img.shape[2] == 4:
            ...
            qimg = QImage(
                img_rgba.data, w, h, img_rgba.strides[0], QImage.Format.Format_RGBA8888
            )
        else:
            if len(img.shape) < 3:
            ...
            h, w = img_rgb.shape[:2]
            qimg = QImage(
                img_rgb.data, w, h, img_rgb.strides[0], QImage.Format.Format_RGB888
            )
