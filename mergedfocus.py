...
class ScalableImageLabel(QLabel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._is_cutoff_active = False # <-- MODIFIED: State for cutoff visibility

    def set_effects_active(self, lighting=False, focus=False, cutoff=False): # <-- MODIFIED
        self._is_lighting_active = lighting
        self._is_focus_active = focus
        self._is_cutoff_active = cutoff # <-- MODIFIED
        self.update()
    ...
    def paintEvent(self, event):
        ...
        if self._is_lighting_active:
            # Draw angle indicator (always visible when lighting is on)
            ...
            # <-- MODIFIED: Only draw the cutoff marker if the feature is active
...
# ... ClickableImageLabel class is unchanged ...
...
class MainApp(QMainWindow):
    def __init__(self):
        ...
        self.light_end_point = None
        self.initUI()


    def initUI(self):
        # ... (most of UI setup is the same) ...
        ...
        self.radio_set_focus = QRadioButton("Set Focus Point")
        self.radio_set_focus.setChecked(True)
        mode_layout.addWidget(self.radio_set_light)
        mode_layout.addWidget(self.radio_set_focus)
        lighting_form.addRow("Click Action:", mode_widget)

        # <-- MODIFIED: Add the cutoff enable checkbox
        self.cutoff_enable_cb = QCheckBox("Enable Light Cutoff")
        self.cutoff_enable_cb.setChecked(True)
        ...
        self.light_blend_combo.currentIndexChanged.connect(self._apply_effects_to_source)
        
        # <-- MODIFIED: Add the new checkbox to the update loop
        self.cutoff_enable_cb.toggled.connect(self._apply_effects_to_source)
        self.focus_enable_cb.toggled.connect(self._apply_effects_to_source)
        ...
        for w in [self.angle_slider_w, self.intensity_slider_w, self.smoothness_slider_w, self.focus_size_slider_w, self.blur_amount_slider_w, self.focus_transition_slider_w]:
            w.findChild(QSlider).valueChanged.connect(self._apply_effects_to_source)
    ...
    # ... (no change to toggle_effects_tab, handle_point_selection, load_source, load_destination, choose_color) ...
    def toggle_effects_tab(self, checked):
        self.tabs.setTabEnabled(1, checked)
        self._apply_effects_to_source()

    def handle_point_selection(self, x, y):
        if not self.effects_master_checkbox.isChecked():
            return
        if self.radio_set_light.isChecked():
            self.light_end_point = (x, y)
        elif self.radio_set_focus.isChecked():
    ...
    def _apply_effects_to_source(self):
        ...
        if is_lighting_master_on:
            processed_image = self._apply_gradient_effect(processed_image)
        
        if is_focus_on:
            processed_image = self._apply_focus_effect(processed_image)
        
        self.src_img_lit = processed_image
        
        # <-- MODIFIED: Pass the cutoff state to the label
        self.src_label.set_effects_active(
            lighting=is_lighting_master_on,
            focus=is_focus_on,
            cutoff=is_cutoff_on
        )
    ...
    def _apply_gradient_effect(self, image):
        ...
        is_cutoff_enabled = self.cutoff_enable_cb.isChecked() # <-- MODIFIED
    ...
    # ... (_apply_focus_effect is unchanged) ...
    ...
    # <-- MODIFIED: Now takes a boolean to decide which gradient logic to use
    def _create_gradient(self, w, h, angle, end_point, color_bgr, intensity, smoothness, is_cutoff_enabled):
        ...
        if is_cutoff_enabled:
            # Logic with user-defined cutoff
            ...
            # Logic for a full, edge-to-edge gradient
            max_p = projection.max()
            denominator = max_p - min_p
            if abs(denominator) < 1e-6: denominator = 1e-6
            # Use (max_p - projection) to keep the light direction consistent with the cutoff mode
    ...
    def run_cloning(self):
        ...
        try:
            ...
            is_oob = ((tl_x<1) or (tl_y<1) or (br_x>=dest_w-1) or (br_y>=dest_h-1))
            if is_oob and clone_mode_str.startswith("Seamless"):
                QMessageBox.information(self, "Mode Switched", "Seamless Clone requires a border.\nSwitching to 'Alpha Blend'.")
            ...
            if (roi_x2-roi_x1)<=0 or (roi_y2-roi_y1)<=0: return
            ...
            if clone_mode_str.startswith("Alpha Blend"):
                ...
                dest_roi = self.dest_img[roi_y1:roi_y2, roi_x1:roi_x2]
                ...
                self.result_img = self.dest_img.copy(); self.result_img[roi_y1:roi_y2, roi_x1:roi_x2] = roi
            else:
                ...
                self.result_img = cv2.addWeighted(cloned, strength, self.dest_img, 1-strength, 0)

            self.display_image(self.result_img, self.result_label)
            if QMessageBox.question(self, "Save Image", "Do you want to save?") == QMessageBox.StandardButton.Yes: self.save_result()
    ...
    def _rotate_image_and_mask(self, image_bgra, mask, angle_deg):
        ...
        rot_img = cv2.warpAffine(image_bgra, M, (new_w,new_h), flags=cv2.INTER_LINEAR, borderValue=(0,0,0,0))
        rot_mask = cv2.warpAffine(mask, M, (new_w,new_h), flags=cv2.INTER_NEAREST, borderValue=0)
