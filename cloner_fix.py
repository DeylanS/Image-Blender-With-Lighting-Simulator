...
class ClickableImageLabel(ScalableImageLabel):
    ...
    def set_preview_dependencies(self, source_label, resize_slider, rotate_slider, feather_slider):
        ...
        # Feather slider doesn't need a live preview update, it's a generation-time effect
...
class SeamlessClonerApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("OpenCV Seamless Cloner"); self.setGeometry(100, 100, 1200, 850)
    ...
    def initUI(self):
        ...
        self.clone_method_combo = QComboBox(); self.clone_method_combo.addItems(["Alpha Blend / Feather", "Seamless - Normal", "Seamless - Mixed"])
        ...
        scale_slider_layout, rotate_slider_layout, strength_slider_layout, feather_slider_layout = QHBoxLayout(), QHBoxLayout(), QHBoxLayout(), QHBoxLayout()
        self.resize_slider, self.rotate_slider, self.strength_slider, self.feather_slider = QSlider(Qt.Orientation.Horizontal), QSlider(Qt.Orientation.Horizontal), QSlider(Qt.Orientation.Horizontal), QSlider(Qt.Orientation.Horizontal)
        self.scale_label, self.rotate_label, self.strength_label, self.feather_label = QLabel(), QLabel(), QLabel(), QLabel()
        
        # Slider setup loop
        sliders = [
            (self.resize_slider, self.scale_label, "Scale:", "%", 10, 200, 100),
            (self.rotate_slider, self.rotate_label, "Rotate:", "°", -180, 180, 0),
            (self.strength_slider, self.strength_label, "Intensity:", "%", 0, 100, 100),
            (self.feather_slider, self.feather_label, "Feather:", "px", 0, 50, 0) # New Feather Slider
        ]
        for slider, label, name, unit, r_min, r_max, r_def in sliders:
            slider.setRange(r_min, r_max); slider.setValue(r_def)
            label.setText(f"{r_def}{unit}")
            slider.valueChanged.connect(lambda v, lbl=label, u=unit: lbl.setText(f"{v}{u}"))

        for layout, label_text, slider, text_label in [(scale_slider_layout, "Scale:", self.resize_slider, self.scale_label), (rotate_slider_layout, "Rotate:", self.rotate_slider, self.rotate_label), (strength_slider_layout, "Intensity:", self.strength_slider, self.strength_label), (feather_slider_layout, "Feather:", self.feather_slider, self.feather_label)]:
            layout.addWidget(QLabel(label_text)); layout.addWidget(slider); layout.addWidget(text_label)

        for layout in [scale_slider_layout, rotate_slider_layout, strength_slider_layout, feather_slider_layout]: sliders_layout.addLayout(layout)
        sliders_layout.insertWidget(0, controls_title)
        
        self.dest_label.set_preview_dependencies(self.src_label, self.resize_slider, self.rotate_slider, self.feather_slider)
    ...
    def load_source(self):
        ...
        if file_path:
            ...
            if len(self.src_img.shape) < 3 or self.src_img.shape[2] != 4: QMessageBox.warning(self, "Warning", "For best results, use a PNG with a transparency layer.")
    ...
    def run_cloning(self):
        ...
        feather_amount = self.feather_slider.value()
        ...
        try:
            ...
            # --- FINAL FIX: Detect if object is too close to the edge for seamless clone ---
            ...
            # --- Unified Cropping Logic (from user's working code) ---
            ...
            if clone_mode_str.startswith("Alpha Blend"):
            ...
            else: # Seamless Modes (now guaranteed to be in-bounds)
                ...
                cloned_at_100 = cv2.seamlessClone(clipped_src, dest_for_clone, clipped_mask, safe_center, mode)
